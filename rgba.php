...
/**
 * Enter the directory in which to store cached PNG images.
 ...
 */
define('PNGS_DIR', ''); //Defaulted to same directory this script is in
...
$dir = substr(PNGS_DIR, 0, strlen(PNGS_DIR) - 1);
...
$filepath = PNGS_DIR . "color_r{$red}_g{$green}_b{$blue}_a{$alpha}.png";

if (CACHE_PNGS && file_exists($filepath))
{
...
{
	$img = @imagecreatetruecolor(PNG_SIZE, PNG_SIZE) or die('Cannot Initialize new GD image stream');
